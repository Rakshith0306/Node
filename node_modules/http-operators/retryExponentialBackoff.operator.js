import { Observable, timer, asyncScheduler, Subscription, } from 'rxjs';
 ...
 */
export const retryExponentialBackoff = (maxTries = -1, initialWait = 1, scheduler = asyncScheduler) => (source) => {
    return new Observable(subscriber => {
        let count = 1;
        const subscription = new Subscription();
        const subscribe = () => subscription.add(source.subscribe({
            next(value) {
                count = 1;
...
//     }
// }
export const retryWhenExponentialbackoff = (maxTries = -1, initialWait = 1) => (source) => {
    return source.pipe((innerSource) => {
        let count = 1;
        return innerSource.pipe(retryWhen((errors) => {
